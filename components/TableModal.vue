<template>
  <div class="text-center">
    <v-dialog
      v-model="dialog"
      width="500"
      persistent
    >
      <template v-slot:activator="{ on, attrs }">
        <v-btn
          color="red lighten-2"
          dark
          v-bind="attrs"
          v-on="on"
          class="table__btn"
        >
          Start sorting!
        </v-btn>
      </template>

      <v-card>
        <v-card-title class="text-h5 grey lighten-2 modal__head--title">
          <v-row class="modal__head">
                  <v-col>
                    <div class="modal__head--title">How many people?</div>
                  </v-col>
                  <v-col class="modal__head--right">
                      <v-btn elevation="0" depressed class="close" @click="dialog = false">
                            <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                <rect width="12" height="12" fill="url(#pattern0)"/>
                                <mask id="mask0_101_7" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="12" height="12">
                                <rect width="12" height="12" fill="url(#pattern1)"/>
                                </mask>
                                <g mask="url(#mask0_101_7)">
                                <rect width="12" height="12" fill="#999999"/>
                                </g>
                                <defs>
                                <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                                <use xlink:href="#image0_101_7" transform="scale(0.0078125)"/>
                                </pattern>
                                <pattern id="pattern1" patternContentUnits="objectBoundingBox" width="1" height="1">
                                <use xlink:href="#image0_101_7" transform="scale(0.0078125)"/>
                                </pattern>
                                <image id="image0_101_7" width="128" height="128" xlink:href="data:image/png;base64,"/>
                                </defs>
                            </svg>
                        </v-btn>
                  </v-col>
              </v-row>
        </v-card-title>
        <v-divider></v-divider>
        <v-card-text>
            <div class="modal__body">Enter a number of how many people you want to add to the list.</div>
            <v-text-field
            autofocus
            type="number"
            :rules="[numberRule]"
            v-model="persons"
            outlined
            @keyup.enter="generatePeople"
        ></v-text-field>
        </v-card-text>
        <v-divider></v-divider>

        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn class="modal__btn--grey" depressed>Cancel</v-btn>
              <v-btn class="modal__btn--primary" depressed @click="generatePeople">Start</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
</template>

<script>
  export default {
    data () {
      return {
        dialog: false,
        persons: null,
        isValid: false,
        msg: 'Number of people has to be between 20 and 100',
        numberRule: v  => {
          this.isValid = false
          if (typeof v !== 'undefined' && v && !isNaN(parseFloat(v)) && v >= 20 && v <= 100){
            this.isValid = true
            return true;
          }
          return this.msg
        },
      }
    },
    created() {
      this.numberRule
    },
    methods: {
      async generatePeople() {
        this.persons = parseInt(this.persons)
        if (this.isValid) {                
          this.$emit('submit', this.persons)
          this.persons = null
          this.dialog = false
        } else {
          alert(this.msg)
        }
      }
    }
  }
</script>

<style lang="scss" scoped>
    .table__btn {
        background: #FF8D00!important;
        font-style: normal;
        font-weight: 700;
        font-size: 15px;
        line-height: 20px;
        color: #FFFFFF;
        text-transform: capitalize !important;
    }
    .modal__head {
        display: flex;
        align-items: center;
        .modal__head--title {
            font-style: normal;
            font-weight: 700;
            font-size: 18px;
            line-height: 21px;
            color: #000000;
        }
        .modal__head--right {
            text-align: right;
        }
    }
    .modal__body {
        font-style: normal;
        font-weight: 400;
        font-size: 13px;
        line-height: 18px;
        color: #555555;
        padding: 20px 0;
    }
    .modal__btn--grey {
        background: #EEEEEE!important;
        border-radius: 5px;
        font-style: normal;
        font-weight: 700;
        font-size: 14px;
        line-height: 16px;
        color: #555555;
        text-transform: capitalize;
        letter-spacing: 0;
        margin-right: 5px;
    }
    .modal__btn--primary {
        background: #FF8D00!important;
        border-radius: 5px;
        font-style: normal;
        font-weight: 700;
        font-size: 14px;
        line-height: 16px;
        color: #FFFFFF;
        text-transform: capitalize;
        letter-spacing: 0;
    }
</style>